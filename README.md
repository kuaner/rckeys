...
**无菜单栏图标**：遥控器 **双击 菜单** 呼出设置对话框——系统保留手势，不可修改（菜单键单击
因此延迟到双击窗口结束才触发，其余键不受影响）；暂停状态下同样有效。「服务」菜单（工具栏 ⋯）
提供暂停/恢复、重载配置、编辑配置文件、清理映射并退出；窗口底栏显示接管状态
（已接管 / 等待遥控器… / 已暂停）。关闭窗口只是隐藏，服务继续运行。

**不要与其他接管工具（Remote Mic 等）同时运行。**

## 设置对话框

遥控器 **双击 菜单** 呼出——一次只面对一个决策的三层结构：
