...
**无菜单栏图标**：遥控器 **双击 TV** 呼出设置对话框——系统保留手势，不可修改（TV 键单击
因此延迟到双击窗口结束才触发，其余键不受影响；菜单单击默认右键、连点常见，故不用菜单）；
暂停状态下同样有效。「服务」菜单（工具栏 ⋯）提供暂停/恢复、重载配置、编辑配置文件、
